...
datafile1 = 'data/aqi_df.csv'
datafile2 = 'data/aqi_df_diff.csv'
...
def get_NO2_plot():
    ...
    # get the line plot
    climate_graph, CurrC = get_graph(data1, field="NO2", op="mean", title="NO2 conc. vs. weeks")
    ...
    color_mapper = LinearColorMapper(palette=palette, low=0, high=np.max(data1['NO2'])*0.5, nan_color='#d9d9d9')
...
def get_PM25_plot():
    ...
    # get the line plot
    climate_graph, CurrC = get_graph(data1, field="PM25", op="mean", title="PM2.5 conc. vs weeks")
    ...
    # Instantiate LinearColorMapper that linearly maps numbers in a range, into a sequence of colors. Input nan_color.
    color_mapper = LinearColorMapper(palette=palette, low=0, high=np.max(data1['PM25'])*0.8, nan_color='#d9d9d9')
...
def get_NO2_plot_diff():
    ...
    # get the line plot
    climate_graph, CurrC = get_graph(data2, field="NO2", op="mean", title="% change in NO2 across weeks of 2020 & 2019")
    ...
    # Input GeoJSON source that contains features for plotting.
    geosource = GeoJSONDataSource(geojson=json_data(5))

    Overall = ColumnDataSource(data2)
    Curr = ColumnDataSource(data2[data2['week'] == 5])
    ...
    # Add Tap tool
    tap_cb = get_callback('tap_climate_diff', args=[Overall, CurrC])
    tap = TapTool(callback=tap_cb)
    tap_cb.args["graph"] = climate_graph
    tap_cb.args["field_name"] = "NO2"
    ...
    # slider object
    slider = Slider(title='Week', bar_color='white', start=5, end=max(merged_df['week']), step=1, value=5, margin=(0,0,0,250), orientation="horizontal", width=300)
    callback = get_callback('NO2_slider', [Overall, Curr])
    slider.js_on_change('value', callback)
    callback.args["slider"] = slider
    callback.args["map"] = p
...
def get_PM25_plot_diff():
    print("PM25", np.min(data2['PM25']))
    ...
    # get the line plot
    climate_graph, CurrC = get_graph(data2, field="PM25", op="mean", title="% change in PM2.5 across weeks of 2020 & 2019")
